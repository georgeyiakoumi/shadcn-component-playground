...
export interface GenerateOptions {
  ...
  /** User edits (M3 from-scratch builder) */
...
/**
 * Entry point — used by the M3 from-scratch builder export flow.
 *
 * Pillar 6 (GEO-291) deleted the M2 `generateFromExisting` branch.
 * Stock-component edit→export now goes through the v2 parser+generator
 * pipeline (`lib/parser/generate-from-tree-v2.ts`) which respects the
 * source byte-for-byte. This function is kept as a thin alias so the
 * existing M3 callers (export-dialog, custom slug page, new page) don't
