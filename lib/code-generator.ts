...
/**
 * Strips any leading `// Generated by shadcn Playground` header from a
 * source string. Handles both the plain one-line form emitted by
 * `generateFromTree` and the two-line `// Component: <name>` form
 * emitted by `generateFromExisting`. Used to prevent double-headers
 * when a source that was already generated by us passes back through
 * the export pipeline.
 */
function stripGeneratedHeader(source: string): string {
  // Match "// Generated by shadcn Playground" optionally followed by
  // "// Component: ..." on the next line, plus any trailing blank line.
...
function generateFromExisting(options: GenerateOptions): string {
  ...
  // Strip any existing "Generated by shadcn Playground" header from the
  // source before we prepend our own. Without this, from-scratch components
  // (whose source already starts with that header from generateFromTree) end
  // up with two "Generated by" lines after passing through the export flow.
  const strippedSource = stripGeneratedHeader(originalSource)
  ...
  if (isUnmodified) {
    return header + strippedSource
  }

  let output = strippedSource
